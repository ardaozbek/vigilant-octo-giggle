...
def edit_member(member_id):
    ...
    db.execute('UPDATE members SET name = ?, email = ?, level = ? WHERE id = ?',\
         [member_data['name'], member_data['email'], member_data['level'], member_id])
    db.commit()

    member_cur = db.execute('SELECT id, name, email, level FROM members WHERE id = ?', [member_id])
    edited_member = member_cur.fetchone()

    return jsonify({'member': {"id": edited_member['id'], "name": edited_member['name'],\
         "email": edited_member['email'], "level": edited_member['level']}})
